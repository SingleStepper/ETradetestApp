// Found at: https://gist.github.com/DeskSupport/2951522#file-oauth-cs
...
namespace OAuth {
   ...
   public class Manager {
      ...
      public Manager() {
         ...
         _params["request_token_secret"]="";
         _params["oauth_request_token_secret"]="";
         _params["version"]="1.0";
      }
      ...
      public OAuthResponse AcquireAccessToken(string uri, string method, string pin,
         out string sentHeader) {
         NewRequest();
         //_params["verifier"]=pin;
         // _params["verify_code"]=pin; / bad request
         _params["verifier"]=pin; // Invalid

         //_params.Remove("callback");

         var authzHeader = GetAuthorizationHeader(uri, method);

         sentHeader=authzHeader;
         // prepare the token request
         var request = (System.Net.HttpWebRequest)System.Net.WebRequest.Create(uri);
         request.Headers.Add("Authorization", authzHeader);
         request.Method=method;
